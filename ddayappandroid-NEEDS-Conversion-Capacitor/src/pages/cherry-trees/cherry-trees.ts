...
import { CherryTreesSearchModalPage } from '../cherry-trees-search-modal/cherry-trees-search-modal';
import { Subscription } from 'rxjs';

@IonicPage()
@Component({
  selector: 'page-cherry-trees',
  templateUrl: 'cherry-trees.html',
})
export class CherryTreesPage {
  ...
  private _subscription:Subscription;
    ...
    ionViewDidEnter(){
      this._subscription = this.searchProvider.getCherryTreesJSON().subscribe(
        result => {
    ...
    treeClicked(item: any) {
      //console.log(item);
      const modal = this.modalCtrl.create(CherryTreesSearchModalPage, {soldier: item});
      modal.present();
    }
